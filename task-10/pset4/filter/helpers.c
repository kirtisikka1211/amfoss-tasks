...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    int totar, totalg, totalb,arrow,arrcol,count;
    int totalr=totalg=totalb=0;
    for(int row =0; row<height; row++)
    {
        for (int col=0, swap= width-1;  col<width; col++)
        {
           for (arrow =row-1; arrow<= row+1;arrow++){
               for(arrcol = col-1; arrcol<=col +1; arrcol++){
                   if(arrow<width && arrow < height && arrcol >=0 && arrow >=0)
                   {
